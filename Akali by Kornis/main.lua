...
local function Combo()
	...
	if menu.combo.rcombo:get() then
		if(target.pos:dist(player) < spellR.range) and not menu.blacklist[target.charName]:get() and(target.pos:dist(player) > menu.combo.minr:get() and player.buff['akalishadowdance'] and player.buff['akalishadowdance'].stacks2 > menu.combo.saver:get()) then
...
local targets = GetTargetGap()
	...
	if menu.combo.gappppr:get() then
		if targets and player:spellSlot(0).state == 0  and (targets.pos:dist(player) < spellR.range+spellQ.range-160) and (targets.pos:dist(player)) > spellR.range and not menu.blacklist[targets.charName]:get() and(targets.pos:dist(player) > menu.combo.minr:get() and player.buff['akalishadowdance'] and player.buff['akalishadowdance'].stacks2 > menu.combo.saver:get()) then
...
function KillSteal()
	local enemy = common.GetEnemyHeroes()
		...
		if enemies and common.IsValidTarget(enemies) and not common.HasBuffType(enemies, 17) then
			...
			if player:spellSlot(0).state == 0 and vec3(enemies.x,enemies.y, enemies.z):dist(player)<spellQ.range and dmglib.GetSpellDamage(0, enemies) > hp then
		...
		if menu.killsteal.kse:get() then
			...
			if menu.killsteal.ksr:get() then
				if player:spellSlot(3).state == 0 and vec3(enemies.x,enemies.y, enemies.z):dist(player)<spellR.range and dmglib.GetSpellDamage(3, enemies) > hp then
					
					player:castSpell("obj", 3, enemies)
				end
			end
			if menu.killsteal.ksgap:get() then
				if player:spellSlot(0).state == 0 and vec3(enemies.x,enemies.y, enemies.z):dist(player)>spellQ.range and vec3(enemies.x,enemies.y, enemies.z):dist(player)<spellQ.range +spellR.range-70 and dmglib.GetSpellDamage(0, enemies) > hp then
	 ...
	 if (player.mana / player.maxMana) * 100  >= menu.laneclear.mana:get() then
		...
		if menu.laneclear.farmlogic:get() then
		   ...
		   if minios then		
					...
					if  minios and EDamage(minios) > minios.health and vec3(minios.x, minios.y, minios.z):dist(player.pos) <= spellE.range then
...
function LastHit()
	...
	for i, minion in pairs(enemyMinions) do
		...
		if minion and not minion.isDead and minion.isVisible and player.pos:dist(minion.pos) < spellQ.range and dmglib.GetSpellDamage(0, minion) >= orb.farm.predict_hp(minion, delay, true) then
...
function EDamage(target)
	...
	if player:spellSlot(2).level > 0 then
	 damage = CalcADmg(target, (ElvlDmg[player:spellSlot(2).level] + --[[Potato Code]]((common.GetBonusAD()-common.GetTotalAD()/2)* .8) + (common.GetTotalAP() * .6)))
	end
	return damage
end
...
function GetClosestMobForLogic()
...
function GetClosestJungleEnemy()
...
--	if menu.draws.drawwr:get() then
	...
	if menu.combo.gappppr:get() then
		if targets  and (targets.pos:dist(player) < spellR.range+spellQ.range-160) and (targets.pos:dist(player)) > spellR.range and not menu.blacklist[targets.charName]:get() and(targets.pos:dist(player) > menu.combo.minr:get() and player.buff['akalishadowdance'] and player.buff['akalishadowdance'].stacks2 > menu.combo.saver:get()) then
			if minion then
